![测试物理帧分配](file-2026-06-15-19--06-83.png)

## 多级页表管理

实现页表数据结构

![](file-2026-06-15-20--06-15.png)

建立和拆除虚实地址之间的联系

![](file-2026-06-15-20--06-96.png)

实现手动查询地址映射

![](file-2026-06-15-20--06-27.png)
...
![](file-2026-06-15-20--06-53.png)

接着，用MapType描述逻辑段内所有虚拟页号映射到物理页帧的方式

![](file-2026-06-15-20--06-77.png)

同时，利用MapPermisssion控制逻辑段的访问方式，其是页表项标志位PTEFlags的子集

![](file-2026-06-15-20--06-19.png)

然后，就可以实现地址空间了，也就是一系列有关联的逻辑段，用MemorySet来表示

![](file-2026-06-15-20--06-19%201.png)

同时，实现MemorySet的方法。

![](file-2026-06-15-20--06-91.png)

上述实现用到了MapArea提供的一些方法，具体实现如下：

![](file-2026-06-15-20--06-42.png)
### 实现内核地址空间

实现创建内核地址空间的方法new_kernel

![](file-2026-06-15-20--06-35.png)

### 实现应用地址空间

修改linker

![](file-2026-06-15-20--06-85.png)

精简loader

![](file-2026-06-15-20--06-38.png)

解析elf格式数据

![](file-2026-06-15-20--06-98.png)

在cargo中添加依赖

![](file-2026-06-15-20--06-20.png)

实现memory_set子模块所添加的代码

在main中添加各类use
![](file-2026-06-15-20--06-99.png)

在config中配置参数
![](file-2026-06-15-20--06-69.png)
...
![](file-2026-06-15-21--06-90.png)

然后，在rust_main中进行内存管理子系统的初始化

![](file-2026-06-15-21--06-03.png)

### 实现跳板机制

首先，扩展Trap上下文

![](file-2026-06-15-21--06-89.png)

实现地址空间切换

![](file-2026-06-15-21--06-15.png)

将 trap.S 中的整段汇编代码放置在 .text.trampoline 段，并在调整内存布局的时候将它对齐到代码段的一个页面中

![](file-2026-06-15-21--06-04.png)

### 加载和执行应用程序

修改任务子模块，更新任务控制块管理

![](file-2026-06-15-21--06-73.png)

![](file-2026-06-15-21--06-83.png)

在内核初始化的时候，需要将所有的应用程序加载到全局应用管理器中，同时，也要修改TaskManager的实现

![](file-2026-06-15-21--06-16.png)

修改switch.S

![](file-2026-06-15-21--06-40.png)

修改switch.rs

![](file-2026-06-15-21--06-73%201.png)
### 改进trap的管理

首先修改init函数。然后再trap_handler的开头增加set_kernel_trap_entry
同时，在处理完trap后还要调用trap_return 返回用户态。

![](file-2026-06-15-21--06-58.png)

同时，在每一个应用程序第一次获得CPU权限时，内核栈顶放置在内核加载应用的时候构造的一个任务上下文

![](file-2026-06-15-21--06-36.png)

### 改进sys_write的实现

由于地址空间的隔离，sys_write无法直接方法应用空间的数据。为此，页表page_table提供一个将应用地址空间的缓冲区转化为内核地址空间可以直接访问的辅助函数

![](file-2026-06-15-22--06-53.png)

修改sys_write实现

![](file-2026-06-15-22--06-75.png)
...
![](file-2026-06-15-22--06-55.png)

#### 删除build.py

因为应用程序的起始地址是相同的了，所以不在需要build.py，直接删除即可

![](file-2026-06-15-22--06-05.png)

同时，修改Makefile文件

![](file-2026-06-15-22--06-10.png)

### 修改main.rs

![](file-2026-06-15-22--06-95.png)

修改build.rs

![](file-2026-06-15-22--06-15.png)
...
![](file-2026-06-15-23--06-60.png)

![](file-2026-06-15-23--06-54.png)
